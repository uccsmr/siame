/**
 * SIRPC · Backend Google Apps Script · HOME RUN V4 · FIX CARGA DOCENTE
...
const SIRPC = {
  // OPCIONAL: pegue aquí el ID del Google Sheets si el Apps Script no fue creado desde el archivo.
  // Si el script sí fue creado desde Extensiones > Apps Script dentro del Google Sheets, déjelo vacío.
  SPREADSHEET_ID: '',
  SHEETS: {
    PLANES: 'PlanesCurso',
    CITAS: 'Citas',
...
function route_(action, payload) {
  if (action === 'ping') return { ok: true, message: 'SIRPC backend V4 activo', timestamp: now_(), spreadsheet: ss_().getName() };
  if (action === 'diagnostico') return diagnostico_(payload);
...
function ss_() {
  if (SIRPC.SPREADSHEET_ID && String(SIRPC.SPREADSHEET_ID).trim()) {
    return SpreadsheetApp.openById(String(SIRPC.SPREADSHEET_ID).trim());
  }
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  if (!ss) throw new Error('No se encontró Spreadsheet activo. Cree este Apps Script desde el Google Sheets o configure SIRPC.SPREADSHEET_ID.');
  return ss;
}
...
function docProfesor_(obj) {
  return pick_(obj, ['Documento Profesor', 'DocumentoProfesor', 'Documento_Profesor', 'Documento', 'Cédula Docente', 'CedulaDocente', 'Cedula Docente']);
}

function correoProfesor_(obj) {
  return pick_(obj, ['Correo-E', 'Correo', 'Correo Institucional', 'CorreoInstitucional', 'Email']);
}

function nombreProfesor_(obj) {
  return pick_(obj, ['Nombre_Completo', 'Nombre Completo', 'NombreCompleto', 'Docente', 'Nombre Profesor']);
}

function idPlan_(obj) {
  return pick_(obj, ['IDPlan', 'ID Plan', 'IdPlan']);
}
...
function diagnostico_(payload) {
  const qRaw = String(payload.query || '').trim();
  const qDoc = normDoc_(qRaw);
  const q = norm_(qRaw);
  const ss = ss_();
  const sh = sheet_(SIRPC.SHEETS.PLANES);
  const hm = headers_(sh);
  const lastRow = sh.getLastRow();
  const planes = rows_(SIRPC.SHEETS.PLANES);
  let coincidencias = [];
  if (qRaw) {
    coincidencias = planes.filter(function(p) {
      return (qDoc && normDoc_(docProfesor_(p)) === qDoc) || norm_(correoProfesor_(p)) === q || norm_(nombreProfesor_(p)).indexOf(q) >= 0;
    }).slice(0, 10).map(function(p) {
      return {
        IDPlan: idPlan_(p),
        Documento: docProfesor_(p),
        Nombre: nombreProfesor_(p),
        Correo: correoProfesor_(p),
        Curso: p['Descripción'] || '',
        Clase: p['Nº Clase'] || '',
  ...
  return {
    ok: true,
    version: 'V4_FIX_CARGA_DOCENTE',
    spreadsheet: ss.getName(),
    spreadsheetId: ss.getId(),
    hojas: ss.getSheets().map(function(s) { return s.getName(); }),
    hojaPlanes: SIRPC.SHEETS.PLANES,
    lastRowPlanes: lastRow,
    headersPlanes: hm.list,
    query: qRaw,
    coincidencias: coincidencias,
    totalCoincidenciasMostradas: coincidencias.length,
    timestamp: now_()
  };
}
...
function buscarPlanes_(payload) {
  ...
  // V4: primero filtra únicamente PlanesCurso. Esto evita leer miles de revisiones
  // cuando el docente aún no tiene citas, que era la causa más común de carga lenta/no carga.
  const todosPlanes = rows_(SIRPC.SHEETS.PLANES);
  const planes = todosPlanes.filter(function(p) {
    if (upper_(p.EstadoPlan) === 'PENDIENTE_DOCENTE') return false;
    const doc = normDoc_(docProfesor_(p));
    const correo = norm_(correoProfesor_(p));
    const nombre = norm_(nombreProfesor_(p));
    return (qDoc && doc === qDoc) || correo === q || (q && nombre.indexOf(q) >= 0);
  });
  ...
  if (planes.length === 0) {
    return {
      ...
      debug: {
        query: qRaw,
        totalPlanesLeidos: todosPlanes.length,
        mensaje: 'No hubo coincidencias en PlanesCurso. Verifique Documento Profesor o Correo-E.'
      }
    };
  }

  const ids = {};
  planes.forEach(function(p) { ids[idPlan_(p)] = true; });

  // Citas suele ser una hoja pequeña; se usa para saber si ya hay agenda activa.
  const citas = rows_(SIRPC.SHEETS.CITAS).filter(function(c) { return ids[c.IDPlan]; });

  // Solo leemos Revisiones si realmente hay citas/historial para esos planes.
  // En base nueva, esto acelera enormemente la consulta del docente.
  let revisiones = [];
  const hayHistorial = citas.length > 0 || planes.some(function(p) {
    return ['EN_PROCESO', 'COMPLETAMENTE_VISADO', 'CON_OBSERVACIONES', 'NO_ASISTIO'].indexOf(upper_(p.EstadoPlan)) >= 0;
  });
  if (hayHistorial) {
    revisiones = rows_(SIRPC.SHEETS.REVISIONES).filter(function(r) { return ids[r.IDPlan]; });
  }

  const planesConEstado = planes.map(function(p) {
    const revs = SIRPC.TIPOS_REVISION.map(function(tipo) {
      return estadoRevisionPlan_(idPlan_(p), tipo, revisiones, citas);
    });
    p.revisiones = revs;
    return p;
  });

  const primero = planesConEstado[0];
  const docente = {
    documento: docProfesor_(primero),
    nombre: nombreProfesor_(primero),
    correo: correoProfesor_(primero)
  };
